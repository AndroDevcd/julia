...
(define initial-reserved-words '(begin while if for try return break continue
                         ...
                         abstract typealias bitstype type immutable
...
(define (parse-call s)
  (let ((ex (parse-unary-prefix s)))
    (if (or (initial-reserved-word? ex) (memq ex '(struct mutable primitive)))
        (parse-resword s ex)
     ...
     (case word
       ...
       ((struct mutable)
        (if (not (eq? (peek-token s) 'type))
            ...
            (begin (take-token s)
                   (parse-struct-def s (eq? word 'mutable) word))))
       ((primitive)
        (if (not (eq? (peek-token s) 'type))
            (parse-call-chain s word #f)
            (begin (take-token s)
       ...
       ((type)
        (syntax-deprecation s "type" "mutable type")
        (parse-struct-def s #t word))
       ((immutable)
        (syntax-deprecation s "immutable" "struct type")
