...
struct type Inner5906{T}
    a:: T
end

struct type Empty5906{T} <: Outer5906{T}
end

struct type Hanoi5906{T} <: Outer5906{T}
    a::T
...
struct type RGB{T<:AbstractFloat} <: Paint{T}
    r::T
    g::T
    b::T
end
...
# issue #7810
mutable type Foo7810{T<:AbstractVector}
    v::T
end
...
# issue #16530
mutable type Foo16530a{dim}
    c::Vector{NTuple{dim, Float64}}
    d::Vector
end
mutable type Foo16530b{dim}
    c::Vector{NTuple{dim, Float64}}
end
...
mutable type HasAbstractlyTypedField
    x::Union{Int,String}
end
...
struct type MyType18457{T,F,G}<:AbstractMyType18457{T,F,G} end
