...
mutable type TestAbstractArray end
...
mutable type T24Linear{T,N,dims} <: AbstractArray{T,N}
...
struct type TSlow{T,N} <: AbstractArray{T,N}
...
let
    mutable type TestThrowNoGetindex{T} <: AbstractVector{T} end
...
mutable type UnimplementedFastArray{T, N} <: AbstractArray{T, N} end
Base.linearindexing(::UnimplementedFastArray) = Base.LinearFast()

mutable type UnimplementedSlowArray{T, N} <: AbstractArray{T, N} end
Base.linearindexing(::UnimplementedSlowArray) = Base.LinearSlow()

mutable type UnimplementedArray{T, N} <: AbstractArray{T, N} end
...
mutable type TSlowNIndexes{T,N} <: AbstractArray{T,N}
    data::Array{T,N}
end
...
mutable type GenericIterator{N} end
