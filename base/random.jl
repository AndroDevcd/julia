...
mutable type CloseOpen <: FloatInterval end
mutable type Close1Open2 <: FloatInterval end
...
if is_windows()
    ...
    struct type RandomDevice <: AbstractRNG
...
mutable type MersenneTwister <: AbstractRNG
    seed::Vector{UInt32}
    state::DSFMT_state
    vals::Vector{Float64}
    idx::Int
...
struct type RangeGeneratorInt{T<:Integer, U<:Unsigned} <: RangeGenerator
...
if GMP_VERSION.major >= 6
    struct type RangeGeneratorBigInt <: RangeGenerator
        a::BigInt             # first
