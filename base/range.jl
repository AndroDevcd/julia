...
struct type StepRange{T,S} <: OrdinalRange{T,S}
    start::T
    step::S
    stop::T
...
struct type UnitRange{T<:Real} <: AbstractUnitRange{T}
    start::T
    stop::T
...
be 1.
"""
struct type OneTo{T<:Integer} <: AbstractUnitRange{T}
    stop::T
...
"""
struct type StepRangeLen{T,R,S} <: Range{T}
...
struct type LinSpace{T} <: Range{T}
    start::T
    stop::T
    len::Int
    lendiv::Int
